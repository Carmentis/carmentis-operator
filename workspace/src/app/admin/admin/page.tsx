...
export default function AdminPage() {
	...
	// load all admins
	useEffect(() => {
		fetch(process.env.NEXT_PUBLIC_WORKSPACE_API_BASE_URL +  '/admin/admin',{})
			.then(res => res.json())
	...
	return <div className={"space-y-4 p-4"}>
		...
		<form className="w-100 card  space-y-2" onSubmit={addAdministrator}>
			<h2><i className={"bi bi-person-add mr-2"}></i>Add an administrator</h2>
			<p>
				Paste the public key of the administrator to allow the administrator
		...
		<div className="card w-100">
			<form className="w-100 mr-2 space-y-2 mb-4" onSubmit={removeAdministrator}>
				<h2><i className={"bi bi-person-dash mr-2"}></i>Remove an administrator</h2>
				<p>
					Paste the public key of the administrator to remove the administrator.
				</p>
				<div className="flex flex-row space-x-2">
					<input type="text" id="publicKey"
